...
import com.androidy.azsecuer.util.FileInfo;
import java.io.File;
...
public class FileSearchManager {
    ...
    private void initDataList() {
        ...
        fileInfos.clear();
        fileInfos.put(FileTypeManager.TYPE_OTHER, new ArrayList<FileInfo>());
        fileInfos.put(FileTypeManager.TYPE_APK, new ArrayList<FileInfo>());
        fileInfos.put(FileTypeManager.TYPE_AUDIO, new ArrayList<FileInfo>());
        fileInfos.put(FileTypeManager.TYPE_IMAGE, new ArrayList<FileInfo>());
        fileInfos.put(FileTypeManager.TYPE_TXT, new ArrayList<FileInfo>());
        fileInfos.put(FileTypeManager.TYPE_VIDEO, new ArrayList<FileInfo>());
        fileInfos.put(FileTypeManager.TYPE_ZIP, new ArrayList<FileInfo>());
        fileSizes.clear();
        fileSizes.put(FileTypeManager.TYPE_OTHER, 0L);
        fileSizes.put(FileTypeManager.TYPE_APK, 0L);
        fileSizes.put(FileTypeManager.TYPE_AUDIO, 0L);
        fileSizes.put(FileTypeManager.TYPE_IMAGE, 0L);
        fileSizes.put(FileTypeManager.TYPE_TXT, 0L);
        fileSizes.put(FileTypeManager.TYPE_VIDEO, 0L);
        fileSizes.put(FileTypeManager.TYPE_ZIP, 0L);
    }
    ...
    private void addFileToList(File file, int romType) {
        String[] iconTypes = FileTypeManager.getFileIconAndType(file);
        String fileType = iconTypes[2];// 文件类型(图像?音频?)
        String iconName = iconTypes[1];// 此文件对应所用图像名称
        String openType = FileTypeManager.getMIMEType(file);// 此文件打开类型
